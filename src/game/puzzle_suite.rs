//! Puzzle suite for measuring engine move-quality across tactical, strategic,
//! and deep positional categories. Run via `chess solve-puzzles`.
...
/// A puzzle with a known best move.
pub struct Puzzle {
    pub fen: &'static str,
...
/// Tier 1 = tactical (depth 6), Tier 2 = BK strategic (depth 10),
...
const TIER_DEPTHS: [u8; 3] = [6, 10, 12];
const TIER_NAMES: [&str; 3] = ["Tactical (d6)", "Strategic (d10)", "Deep Positional (d12)"];
...
/// Run the full puzzle suite once, printing per-puzzle and per-tier results.
pub fn run_puzzle_suite(tier_filter: Option<u8>) {
    println!("Chess Engine Puzzle Suite");
    println!("{}", "=".repeat(70));

    let mut overall_solved = 0;
    let mut overall_total = 0;
    let overall_start = Instant::now();

    for (i, &tier) in [1u8, 2, 3].iter().enumerate() {
        ...
        let depth = TIER_DEPTHS[i];
        let tier_puzzles: Vec<&Puzzle> = PUZZLES.iter().filter(|p| p.tier == tier).collect();
        ...
        for puzzle in &tier_puzzles {
            ...
            let (got, solved) = match result {
                Ok(best) => {
                    let uci = best.to_uci();
                    let ok = uci == puzzle.best_move;
                    (uci, ok)
                }
        ...
        let tier_solved = results.iter().filter(|r| r.solved).count();

        for r in &results {
            let status = if r.solved { "PASS" } else { "FAIL" };
            let move_info = if r.solved {
                r.got.clone()
            } else {
                format!("{} (expected {})", r.got, r.expected)
            };
            println!(
                "  [{:>4}] {:.<40} {} ({:.1}s)",
                status,
        ...
        println!(
            "  Result: {}/{} solved in {:.1}s",
            tier_solved,
            tier_puzzles.len(),
            tier_elapsed.as_secs_f64()
        );

        overall_solved += tier_solved;
        overall_total += tier_puzzles.len();
    }
